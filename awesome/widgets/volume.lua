...
-- Sink #6 working for my Samsung Notebook. Originally, this script called for Sink #0

-- Functions to fetch volume information (pulseaudio)
function get_volume() -- returns the volume as a float (1.0 = 100%)
    local fd = io.popen("pactl list | grep -A 9001 'Sink #6' | grep Volume | head -n 1 | awk -F / '{print $2}' | sed 's/[^0-9]*//g'")
...
function get_mute() -- returns a true value if muted or a false value if not
    fd = io.popen("pactl list | grep -A 9001 'Sink #6' | grep Mute | head -n 1")
...
function create_volume_widget()
    ...
    volume_widget:set_border_color('#66666600')
